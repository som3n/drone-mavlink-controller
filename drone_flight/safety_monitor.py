...
def safety_monitor(master, cfg, flight_start_time):
    ...
    while not abort_mission.is_set():
        ...
        if ax > safety["crash_accel_g"] or ay > safety["crash_accel_g"] or az > safety["crash_accel_g"]:
        ...
        if volts is not None and volts > 1.0:
            min_voltage = safety.get("battery_min_voltage", cfg["flight"].get("battery_min_voltage", 10.5))
